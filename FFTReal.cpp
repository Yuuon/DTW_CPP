...
:	_length (length)
,	_nbr_bits (int (floor (log ((double) length) / log (2.) + 0.5)))
,	_bit_rev_lut (int (floor (log ((double) length) / log (2.) + 0.5)))
,	_trigo_lut (int (floor (log ((double) length) / log (2.) + 0.5)))
...
FFTReal::TrigoLUT::TrigoLUT (const int nbr_bits)
{
	...
	if (nbr_bits > 3)
	{
		...
		for (int level = 3; level < nbr_bits; ++level)
		{
			...
			const float	mul = PI / (level_len << 1);
